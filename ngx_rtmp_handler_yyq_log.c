...
ngx_rtmp_recv(ngx_event_t *rev)
{
    ...
    for( ;; ) {
        ...
        if (old_size) {
            ...
            // yyq add for get time <<<<
            ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
                        "ngx_rtmp_recv s->in_csid = %d, c->recv n = %d", s->in_csid, n);   
            // >>>> yyq add for get time
        ...
        ngx_rtmp_header_t *h, ngx_chain_t *in)
{
    ...
    ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
                          "rtmp_rec_msg FIN. begin at %d us, end = %d us, h->type = %d, h->timestamp = %d",
                          tv_begin.tv_usec, tv_end.tv_usec, h->type, h->timestamp);   
    // >>>> yyq add for get time
